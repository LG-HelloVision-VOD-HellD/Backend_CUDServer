...
@router.post('/{user_id}')
def insert_review(user_id: int, VOD_ID: int):
    result = insert_likeinfo(user_id, VOD_ID)
    if result:
...
def update_review(user_id: int, VOD_ID: int):
    result = delete_likeinfo(user_id, VOD_ID)
    if result:
